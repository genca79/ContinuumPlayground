...
const PLAYGROUND_V2_KEY = 'genca_playground_v2';
const PLAYGROUND_SCENES_KEY = 'genca_playground_scenes_v1';
const PG_ONBOARD_SEEN_KEY = 'genca_pg_onboard_seen_v1';
...
const state = {
    ...
    playground: {
        ...
        advanced: {
            ...
            droneLevel: 0.6,
            audioFollow: true,
            audioFollowDepth: 0.55,
            loopQuantizeStrength: 0.35
        },
...
const clearPlaygroundNow = () => {
    ...
    if (Playground.clearTubes) Playground.clearTubes();
};
const schedulePlaygroundClearAfterFade = () => {
    if (!window.Playground) return;
    if (playgroundFadeTimer) {
        clearTimeout(playgroundFadeTimer);
...
function positionDualSplitLine() {
    if (!els.dualSplitLine || !canvas) return;
    const rect = canvas.getBoundingClientRect();
    const midY = rect.top + (rect.height / 2);
    els.dualSplitLine.style.top = `${midY}px`;
    els.dualSplitLine.style.left = `${rect.left}px`;
    els.dualSplitLine.style.width = `${rect.width}px`;
    els.dualSplitLine.style.right = 'auto';
}
...
function applyImportedMelody(notes, statusEl, label = 'Imported', features = null, meta = null, durations = null, options = null) {
    if (!Array.isArray(notes) || !notes.length) {
        if (statusEl) statusEl.textContent = 'No melody found.';
        return false;
    }
    const snappedNotes = options?.skipSnap ? notes : snapImportedMelodyNotes(notes);
...
function bindUI() {
    ...
    if (uiToggle) {
        uiToggle.onclick = () => {
            const isOpen = els.ui.classList.toggle('active');
            document.body.classList.toggle('ui-open', isOpen);
            refreshLayout();
        };
    }
    ...
    if (els.fadeBtn) els.fadeBtn.onclick = () => {
        const z = getActiveEditZoneId();
        fadeOutZone(z);
        schedulePlaygroundClearAfterFade();
    };
...
function drawActiveTouches(_fadeMul, _fadeDrop, height, nowAudio = null) {
    state.activeTouches.forEach(t => {
        ...
        const visual = getVisualModeFlags(getZoneAudioMode(zoneId));
        const mode = visual.layer ? 'layer' : (visual.sampler ? 'sampler' : 'wt');
        let touchAlpha = getTouchVisualAlpha(t, mode, nowAudio);
        if (t.isArp && t.arpNotes && t.arpNotes.length) {
            touchAlpha = Math.max(touchAlpha, 0.7);
        }
...
function updateToggleLabels() {
    ...
    document.body.classList.toggle('ui-open', els.ui.classList.contains('active'));
...
function clampPlaygroundState(pg) {
    const safe = deepMergePlain(getDefaultPlaygroundState(), pg || {});
    const c = (v, min, max, fb) => Math.max(min, Math.min(max, Number.isFinite(Number(v)) ? Number(v) : fb));
    safe.macro.energy = c(safe.macro.energy, 0, 1, 0.6);
    safe.macro.chaos = c(safe.macro.chaos, 0, 1, 0.4);
    safe.macro.gravity = c(safe.macro.gravity, 0, 1, 0.8);
    safe.macro.density = c(safe.macro.density, 0, 1, 0.5);
    safe.macro.wind = c(safe.macro.wind, -1, 1, 0);
    safe.macro.stream = !!safe.macro.stream;
    safe.advanced.collisionThreshold = c(safe.advanced.collisionThreshold, 0.3, 3, 1.2);
    safe.advanced.mpeSmoothingMs = c(safe.advanced.mpeSmoothingMs, 0, 240, 24);
    safe.advanced.noteHoldMsMin = c(safe.advanced.noteHoldMsMin, 40, 2000, 120);
    safe.advanced.noteHoldMsMax = c(safe.advanced.noteHoldMsMax, safe.advanced.noteHoldMsMin + 20, 3000, 900);
    safe.advanced.maxVoices = c(safe.advanced.maxVoices, 4, 64, 24);
    safe.advanced.pitchQuantize = safe.advanced.pitchQuantize !== false;
    safe.advanced.internalSynthSafe = safe.advanced.internalSynthSafe !== false;
    safe.advanced.timingGridMs = c(safe.advanced.timingGridMs, 0, 2000, 0);
    safe.advanced.swing = c(safe.advanced.swing, 0, 0.6, 0);
    safe.advanced.maxNotesPerSec = c(safe.advanced.maxNotesPerSec, 0, 200, 0);
    safe.advanced.pitchLanes = c(safe.advanced.pitchLanes, 0, 48, 0);
    safe.advanced.anchorNotes = safe.advanced.anchorNotes === true;
    safe.advanced.adaptiveMpe = safe.advanced.adaptiveMpe !== false;
    safe.advanced.impactToTimbre = c(safe.advanced.impactToTimbre, 0, 1, 0.35);
    safe.advanced.noteFlash = safe.advanced.noteFlash !== false;
    safe.advanced.trailByDegree = safe.advanced.trailByDegree === true;
    safe.advanced.showRotationLine = safe.advanced.showRotationLine === true;
    safe.advanced.rollingArp = safe.advanced.rollingArp !== false;
    safe.advanced.harmonicRain = safe.advanced.harmonicRain !== false;
    safe.advanced.frictionDrone = safe.advanced.frictionDrone !== false;
    safe.advanced.rollMinAngularVel = c(safe.advanced.rollMinAngularVel, 0, 1, 0.06);
    safe.advanced.rollDebounceMs = c(safe.advanced.rollDebounceMs, 0, 1000, 90);
    safe.advanced.rollVelocityFloor = c(safe.advanced.rollVelocityFloor, 0, 1, 0.02);
    safe.advanced.droneMaxVoices = c(safe.advanced.droneMaxVoices, 0, 32, 12);
    safe.advanced.droneGainMax = c(safe.advanced.droneGainMax, 0, 0.5, 0.12);
    safe.advanced.droneLevel = c(safe.advanced.droneLevel, 0, 1, 0.6);
    safe.advanced.audioFollow = safe.advanced.audioFollow !== false;
    safe.advanced.audioFollowDepth = c(safe.advanced.audioFollowDepth, 0, 1, 0.55);
    safe.advanced.loopQuantizeStrength = c(safe.advanced.loopQuantizeStrength, 0, 1, 0.35);
    safe.mode = (safe.mode === 'tubes') ? 'tubes' : 'physics';
    safe.tubes = (safe.tubes && typeof safe.tubes === 'object') ? safe.tubes : {};
    safe.tubes.tool = ['select', 'place', 'play', 'move', 'rotate', 'cut', 'delete'].includes(String(safe.tubes.tool || 'select')) ? String(safe.tubes.tool) : 'select';
    safe.tubes.material = ['PVC', 'BAMBOO', 'ALUMINUM', 'STEEL', 'GLASS', 'WOOD'].includes(String(safe.tubes.material || 'PVC')) ? String(safe.tubes.material) : 'PVC';
    safe.tubes.surface = ['smooth', 'corrugated'].includes(String(safe.tubes.surface || 'corrugated')) ? String(safe.tubes.surface) : 'corrugated';
    safe.tubes.length = c(safe.tubes.length, 80, 900, 220);
    safe.tubes.diameter = c(safe.tubes.diameter, 12, 100, 34);
    safe.tubes.angleDeg = c(safe.tubes.angleDeg, -180, 180, 0);
    safe.tubes.endMode = safe.tubes.endMode === 'closed' ? 'closed' : 'open';
    safe.tubes.quantize = safe.tubes.quantize !== false;
    safe.tubes.quantizeStrength = c(safe.tubes.quantizeStrength, 0, 1, 0.7);
    safe.tubes.harmonicSpread = safe.tubes.harmonicSpread !== false;
    safe.tubes.physicalModel = safe.tubes.physicalModel !== false;
    safe.tubes.partialCount = c(safe.tubes.partialCount, 4, 16, 10);
    safe.tubes.physicalMix = c(safe.tubes.physicalMix, 0, 1, 0.65);
    safe.tubes.inharmonicity = c(safe.tubes.inharmonicity, 0, 0.03, 0.006);
    safe.tubes.coupling = c(safe.tubes.coupling, 0, 0.6, 0.18);
    safe.tubes.snapAngleDeg = c(safe.tubes.snapAngleDeg, 0, 45, 0);
    safe.tubes.mood = c(safe.tubes.mood, 0, 1, 0.5);
    safe.tubes.yExpressivity = c(safe.tubes.yExpressivity, 0, 1, 0.6);
    safe.tubes.mute = safe.tubes.mute === true;
    safe.tubes.windPlay = safe.tubes.windPlay !== false;
    safe.tubes.maxVoices = c(safe.tubes.maxVoices, 2, 16, 6);
    safe.tubes.maxCount = c(safe.tubes.maxCount, 4, 64, 24);
    const shape = String(safe.advanced.spawnShape || 'auto');
    safe.advanced.spawnShape = ['auto', 'circle', 'polygon', 'bar'].includes(shape) ? shape : 'auto';
    safe.interaction.autoSpawnIntervalMs = c(safe.interaction.autoSpawnIntervalMs, 40, 600, 180);
    safe.interaction.pointerTrail = safe.interaction.pointerTrail !== false;
    safe.interaction.mode = (String(safe.interaction.mode || 'drawFling') === 'drawWalls') ? 'drawWalls' : 'drawFling';
    safe.enabled = !!safe.enabled;
    return safe;
}

function loadPlaygroundStateFromStorage() {
    try {
        const raw = localStorage.getItem(PLAYGROUND_V2_KEY);
        if (!raw) return getDefaultPlaygroundState();
        return clampPlaygroundState(JSON.parse(raw));
    } catch (_) {
        return getDefaultPlaygroundState();
    }
}

function savePlaygroundStateToStorage() {
    try {
        localStorage.setItem(PLAYGROUND_V2_KEY, JSON.stringify(clampPlaygroundState(state.playground)));
    } catch (_) {}
}

document.addEventListener('DOMContentLoaded', () => {
    if (typeof state !== 'undefined') {
        state.playground = clampPlaygroundState(deepMergePlain(getDefaultPlaygroundState(), state.playground || {}));
        const stored = loadPlaygroundStateFromStorage();
        state.playground = clampPlaygroundState(deepMergePlain(state.playground, stored));
        window.state = state;
    }

    if (!window.Playground) {
        console.error('Playground.js non caricato!');
        return;
    }
    ...
    const btnSpawn = document.getElementById('btnSpawnObject');
    const btnClear = document.getElementById('pgClearScene');
    const windQuickBtn = document.getElementById('pgWindQuick');
    const tubeEditorToggleBtn = document.getElementById('pgTubeEditorToggle');
    const pitchModeBtn = document.getElementById('pgPitchMode');
    const synthSafeBtn = document.getElementById('pgSynthSafe');
    const audioFollowBtn = document.getElementById('pgAudioFollow');
    const anchorNotesBtn = document.getElementById('pgAnchorNotes');
    const rollingArpBtn = document.getElementById('pgRollingArp');
    const harmonicRainBtn = document.getElementById('pgHarmonicRain');
    const frictionDroneBtn = document.getElementById('pgFrictionDrone');
    const adaptiveMpeBtn = document.getElementById('pgAdaptiveMpe');
    const noteFlashBtn = document.getElementById('pgNoteFlash');
    const trailDegreeBtn = document.getElementById('pgTrailDegree');
    const rotationLineBtn = document.getElementById('pgRotationLine');
    const drawWallsBtn = document.getElementById('pgDrawWalls');
    const clearWallsBtn = document.getElementById('pgClearWalls');
    ...
    const setPgTab = (key) => {
        if (!key) return;
        pgTabButtons.forEach((btn) => {
            const active = btn.dataset.pgTab === key;
    ...
    const voicesEl = document.getElementById('pgActiveVoices');
    const cpuEl = document.getElementById('pgCpuHint');
    const playgroundCanvas = document.getElementById('playgroundCanvas');
    const tubeTemplateBtns = Array.from(document.querySelectorAll('.tube-template[draggable="true"]'));

    const controls = {
        energy: document.getElementById('pgMacroEnergy'),
        chaos: document.getElementById('pgMacroChaos'),
        gravity: document.getElementById('pgMacroGravity'),
        density: document.getElementById('pgMacroDensity'),
        wind: document.getElementById('pgMacroWind'),
        collisionThreshold: document.getElementById('pgCollisionThreshold'),
        mpeSmoothingMs: document.getElementById('pgMpeSmoothing'),
        noteHoldMsMin: document.getElementById('pgNoteHoldMin'),
        noteHoldMsMax: document.getElementById('pgNoteHoldMax'),
        maxVoices: document.getElementById('pgMaxVoices'),
        spawnShape: document.getElementById('pgSpawnShape'),
        tubeMaterial: document.getElementById('pgTubeMaterial'),
        tubeSurface: document.getElementById('pgTubeSurface'),
        tubeLength: document.getElementById('pgTubeLength'),
        tubeDiameter: document.getElementById('pgTubeDiameter'),
        tubeEndMode: document.getElementById('pgTubeEndMode'),
        tubeMaxVoices: document.getElementById('pgTubeMaxVoices'),
        tubeMaxCount: document.getElementById('pgTubeMaxCount'),
        tubeQuantizeStrength: document.getElementById('pgTubeQuantizeStrength'),
        tubeSnapAngle: document.getElementById('pgTubeSnapAngle'),
        tubeMood: document.getElementById('pgTubeMood'),
        tubeYExpress: document.getElementById('pgTubeYExpress'),
        tubeMute: document.getElementById('pgTubeMute'),
        audioFollowDepth: document.getElementById('pgAudioFollowDepth'),
        timingGrid: document.getElementById('pgTimingGrid'),
        swing: document.getElementById('pgSwing'),
        maxNotes: document.getElementById('pgMaxNotes'),
        pitchLanes: document.getElementById('pgPitchLanes'),
        loopQuantizeStrength: document.getElementById('pgLoopQuantizeStrength'),
        impactTimbre: document.getElementById('pgImpactTimbre'),
        droneLevel: document.getElementById('pgDroneLevel')
    };

    const playabilityGrid = document.getElementById('pgPlayabilityGrid');
    const macrosGrid = document.getElementById('pgMacrosGrid');
    const playabilityLegend = document.getElementById('pgPlayabilityLegend');
    const settingsLegend = document.getElementById('pgSettingsLegend');
    const playabilityLegendDefault = playabilityLegend ? playabilityLegend.textContent : '';

    const bindLegend = (container, legendEl, fallback = '') => {
        if (!container || !legendEl) return;
        const resolveLegendSource = (target) => {
            if (!target) return null;
            const direct = target.closest('[data-legend], [title], [aria-label]');
            if (direct) return direct;
            const control = target.closest('.hud-control');
            if (!control) return null;
            return control.querySelector('[data-legend], [title], [aria-label]');
        };
        const update = (target) => {
            const source = resolveLegendSource(target);
            if (!source) return;
            const text = source?.dataset?.legend || source.getAttribute('title') || source.getAttribute('aria-label');
            if (text) legendEl.textContent = text;
        };
        const reset = () => { legendEl.textContent = fallback; };
        container.addEventListener('mouseover', (e) => update(e.target));
        container.addEventListener('focusin', (e) => update(e.target));
    ...
    const buildMelodyFromPgEvents = (events) => {
        if (!Array.isArray(events) || !events.length) return false;
        const stepMs = getMelodyStepMs();
        const strength = Math.max(0, Math.min(1, Number(state.playground?.advanced?.loopQuantizeStrength ?? 0.35)));
        const quantizeTime = (ms) => {
            if (!Number.isFinite(ms) || stepMs <= 0) return 0;
            const targetIdx = Math.round(ms / stepMs);
            const targetMs = targetIdx * stepMs;
            return ms + (targetMs - ms) * strength;
        };
        const maxMs = events.reduce((m, ev) => Math.max(m, (ev.timeMs || 0) + (ev.durationMs || 0)), 0);
        const steps = Math.max(4, Math.min(512, Math.round(maxMs / stepMs) || 4));
        const notes = new Array(steps).fill(null);
        const durations = new Array(steps).fill(1);
        const velocity = new Array(steps).fill(null);

        events.forEach((ev) => {
            const adjTime = quantizeTime(ev.timeMs || 0);
            const idx = Math.max(0, Math.min(steps - 1, Math.round(adjTime / stepMs)));
            const durSteps = Math.max(1, Math.min(16, Math.round((ev.durationMs || stepMs) / stepMs)));
            durations[idx] = Math.max(durations[idx] || 1, durSteps);
            const v = Math.max(1, Math.min(127, Math.round(ev.velocity || 80)));
            velocity[idx] = Number.isFinite(velocity[idx]) ? Math.max(velocity[idx], v) : v;
            const existing = getMelodyStepNotes(notes[idx]);
            const next = existing.includes(ev.note) ? existing : [...existing, ev.note];
            notes[idx] = packMelodyStepNotes(next);
        });

        const features = { velocity };
        applyImportedMelody(notes, els.melodyStatusLogInline || els.melodyStatusLog, 'Playground Loop', features, null, durations, { skipSnap: true });
        state.melody.enabled = true;
        if (typeof updateMelodyToggleUI === 'function') updateMelodyToggleUI();
        if (typeof updateMelodyStatusUI === 'function') updateMelodyStatusUI();
        if (!state.melody.running) startMelodyGenerator();
        else restartMelodyGenerator();
        return true;
    };

    const collectPgLoopEventsForMelody = () => {
        const events = [];
        (pgLoopState.loops || []).forEach(loop => {
            if (!loop || !Array.isArray(loop.events)) return;
            loop.events.forEach(ev => events.push({ ...ev }));
        });
        if (pgLoopState.recording && Array.isArray(pgLoopState.events)) {
            pgLoopState.events.forEach(ev => events.push({ ...ev }));
        }
        return events;
    };

    const stopAllPgLoops = () => {
        (pgLoopState.loops || []).forEach(loop => stopPgLoopPlayback(loop));
        pgLoopState.loops = [];
        resetPgLoopRecording();
        if (Playground?.clearLoopGhosts) Playground.clearLoopGhosts();
        updatePgLoopButton();
    };
    ...
    const flushPgLoopsToMelody = () => {
        ...
        const ok = buildMelodyFromPgEvents(events);
        stopAllPgLoops();
        return ok;
    };

    const updatePgLoopButton = () => {
        ...
        if (pgLoopClearBtn) {
            ...
            pgLoopClearBtn.classList.toggle('is-disabled', !hasLoops);
        }
    };

    window.onPlaygroundNote = (ev) => {
        if (!pgLoopState.armed) return;
        const now = Number.isFinite(ev?.timeMs) ? ev.timeMs : performance.now();
        if (!pgLoopState.recording) {
    ...
    const tubeWindPlayBtn = document.getElementById('pgTubeWindPlay');
    const tubeMuteBtn = document.getElementById('pgTubeMute');

    bindLegend(playabilityGrid, playabilityLegend, playabilityLegendDefault);
    bindLegend(advancedPanel, settingsLegend, settingsLegend ? settingsLegend.textContent : "");
        const tubeClearBtn = document.getElementById('pgTubeClear');
    const tubeToolButtons = Array.from(document.querySelectorAll('.tube-tool-btn[data-tube-tool]'));
    const pgPresetButtons = Array.from(document.querySelectorAll('.pg-preset-btn[data-pg-preset]'));
    const pgSceneSelect = document.getElementById('pgSceneSelect');
    const pgSceneName = document.getElementById('pgSceneName');
    const pgSceneSaveBtn = document.getElementById('pgSceneSave');
    const pgSceneLoadBtn = document.getElementById('pgSceneLoad');
    const pgSceneDeleteBtn = document.getElementById('pgSceneDelete');
    const pgSceneExportBtn = document.getElementById('pgSceneExport');
    const pgSceneImportBtn = document.getElementById('pgSceneImport');
    const pgSceneFile = document.getElementById('pgSceneFile');
    const pgSceneCurrentEl = document.getElementById('pgSceneCurrent');
    const pgSceneSavedAtEl = document.getElementById('pgSceneSavedAt');
    const pgOnboard = document.getElementById('pgOnboard');
    const pgOnboardBody = document.getElementById('pgOnboardBody');
    const pgOnboardPrev = document.getElementById('pgOnboardPrev');
    const pgOnboardNext = document.getElementById('pgOnboardNext');
    const pgOnboardDone = document.getElementById('pgOnboardDone');
    const pgOnboardClose = document.getElementById('pgOnboardClose');
    const pgAudioFollowRuntime = {
        armed: false,
        baseline: null
    };
    let tubeEditorOpen = false;
    let armedTubeTemplate = null;
    let armedTubeTemplateBtn = null;
    let suppressPlaygroundBootSpawn = false;
    let currentPlaygroundSceneName = '';

    const setTubeEditorVisible = (force) => {
        const inTubeMode = state.playground?.mode === 'tubes';
        const enabled = !!state.playground?.enabled;
        if (typeof force === 'boolean') tubeEditorOpen = force;
        const visible = !!(tubeEditorPanel && enabled && inTubeMode && tubeEditorOpen);
        if (tubeEditorPanel) tubeEditorPanel.classList.toggle('hidden', !visible);
        if (tubeEditorToggleBtn) {
            tubeEditorToggleBtn.classList.toggle('toggle-on', visible);
            tubeEditorToggleBtn.classList.toggle('toggle-off', !visible);
            tubeEditorToggleBtn.title = visible ? 'Hide tube options' : 'Open tube options';
            tubeEditorToggleBtn.setAttribute('aria-label', tubeEditorToggleBtn.title);
        }
        return visible;
    };

    const parseTubeTemplateFromEl = (el) => {
        if (!el) return null;
        const pInt = (v, d) => {
        ...
        return {
            material: String(el.dataset.material || state.playground.tubes.material || 'PVC'),
            surface: String(el.dataset.surface || state.playground.tubes.surface || 'corrugated'),
            length: pInt(el.dataset.length, state.playground.tubes.length || 220),
            diameter: pInt(el.dataset.diameter, state.playground.tubes.diameter || 34),
            endMode: String(el.dataset.endMode || state.playground.tubes.endMode || 'open')
        };
    };

    const setArmedTubeTemplate = (template, btn = null) => {
        armedTubeTemplate = template;
        if (armedTubeTemplateBtn) armedTubeTemplateBtn.classList.remove('is-armed');
        armedTubeTemplateBtn = btn || null;
        if (armedTubeTemplateBtn) armedTubeTemplateBtn.classList.add('is-armed');
    };
    const loadPlaygroundScenes = () => {
        try {
            const raw = localStorage.getItem(PLAYGROUND_SCENES_KEY);
            const parsed = safeParseJson(raw, {});
    ...
    let playgroundScenes = loadPlaygroundScenes();
    const formatSceneTimestamp = (ts) => {
        const t = Number(ts);
    ...
    const updatePlaygroundSceneStatus = () => {
        const entry = currentPlaygroundSceneName ? playgroundScenes?.[currentPlaygroundSceneName] : null;
        if (pgSceneCurrentEl) pgSceneCurrentEl.textContent = `Scene: ${currentPlaygroundSceneName || '--'}`;
        if (pgSceneSavedAtEl) pgSceneSavedAtEl.textContent = `Saved: ${formatSceneTimestamp(entry?.savedAt)}`;
    };
    const savePlaygroundScenes = () => {
        try {
            localStorage.setItem(PLAYGROUND_SCENES_KEY, JSON.stringify(playgroundScenes || {}));
        } catch (_) {}
    };
    const refreshPlaygroundSceneSelect = (selected = '') => {
        if (!pgSceneSelect) return;
        const names = Object.keys(playgroundScenes || {}).sort((a, b) => a.localeCompare(b, 'it'));
        pgSceneSelect.innerHTML = '<option value="">-- Select Scene --</option>';
        names.forEach((name) => {
            const opt = document.createElement('option');
            opt.value = name;
            opt.textContent = name;
            pgSceneSelect.appendChild(opt);
        });
        if (selected && names.includes(selected)) pgSceneSelect.value = selected;
        updatePlaygroundSceneStatus();
    };
    const PG_ONBOARD_STEPS = [
        'Physics Mode attiva un canvas secondario: usa Rain/Spawn/Clear per generare eventi sonori da collisione.',
        'I materiali cambiano fisica + suono. Energy/Chaos/Gravity/Density/Wind modellano la scena in tempo reale.',
        'In Tubes mode puoi usare SEL/PLC/PLY/CUT/DEL e trascinare template rapidi PVC/ALU/BAM/GLA.',
        'Snapshot e Preset: salva scene complete e richiama configurazioni pronte per performance live.'
    ];
    let pgOnboardStep = 0;
    const setPgOnboardVisible = (visible) => {
        if (!pgOnboard) return;
        pgOnboard.classList.toggle('hidden', !visible);
    };
    const renderPgOnboardStep = () => {
        if (!pgOnboardBody) return;
        const idx = Math.max(0, Math.min(PG_ONBOARD_STEPS.length - 1, pgOnboardStep));
        pgOnboardBody.textContent = `Step ${idx + 1}/${PG_ONBOARD_STEPS.length} - ${PG_ONBOARD_STEPS[idx]}`;
        if (pgOnboardPrev) pgOnboardPrev.disabled = idx === 0;
        if (pgOnboardNext) pgOnboardNext.disabled = idx >= (PG_ONBOARD_STEPS.length - 1);
    };
    const showPlaygroundOnboarding = () => {
        pgOnboardStep = 0;
        renderPgOnboardStep();
        setPgOnboardVisible(true);
    };
    const closePlaygroundOnboarding = (markSeen = true) => {
        setPgOnboardVisible(false);
        if (markSeen) {
            try { localStorage.setItem(PG_ONBOARD_SEEN_KEY, '1'); } catch (_) {}
        }
    };

    const spawnTubeAtClientPoint = (clientX, clientY, template = null) => {
        if (!Playground?.addTube || !playgroundCanvas) return false;
        if (!state.playground?.enabled) return false;
        if (state.playground.mode !== 'tubes') {
            state.playground.mode = 'tubes';
            applyConfig();
        }
        const windNow = Number(state.playground?.macro?.wind || 0);
        const windActive = Math.abs(windNow) > 0.08;
        const defaultAngleRad = windActive
            ? (windNow >= 0 ? 0 : Math.PI)
            : (((Number(state.playground?.tubes?.angleDeg) || 0) * Math.PI) / 180);
        const rect = playgroundCanvas.getBoundingClientRect();
        const x = ((clientX - rect.left) * playgroundCanvas.width) / Math.max(1, rect.width);
        const y = ((clientY - rect.top) * playgroundCanvas.height) / Math.max(1, rect.height);
        const t = template || {};
        const spec = {
            x: Math.max(10, Math.min(playgroundCanvas.width - 10, x)),
            y: Math.max(10, Math.min(playgroundCanvas.height - 10, y)),
            material: String(t.material || state.playground.tubes.material || 'PVC'),
            surface: String(t.surface || state.playground.tubes.surface || 'corrugated'),
            length: Math.max(80, Math.min(480, Number(t.length || state.playground.tubes.length || 220))),
            diameter: Math.max(12, Math.min(100, Number(t.diameter || state.playground.tubes.diameter || 34))),
            endMode: t.endMode === 'closed' ? 'closed' : 'open',
            quantize: state.playground.tubes.quantize !== false,
            windPlay: windActive ? true : state.playground.tubes.windPlay !== false,
            angle: Number.isFinite(Number(t.angle)) ? Number(t.angle) : defaultAngleRad
        };
        const added = Playground.addTube(spec);
        if (added && Playground?.exciteTube) {
            Playground.exciteTube(added.id, { type: 'strike', x: spec.x, y: spec.y, energy: 0.35 });
        }
        return !!added;
    };
    ...
    const syncHudFromState = () => {
        ...
        if (controls.chaos) controls.chaos.value = String(pg.macro.chaos);
        if (controls.gravity) controls.gravity.value = String(pg.macro.gravity);
        if (controls.density) controls.density.value = String(pg.macro.density);
        if (controls.wind) controls.wind.value = String(pg.macro.wind ?? 0);
        if (controls.collisionThreshold) controls.collisionThreshold.value = String(pg.advanced.collisionThreshold);
        if (controls.mpeSmoothingMs) controls.mpeSmoothingMs.value = String(pg.advanced.mpeSmoothingMs);
        if (controls.noteHoldMsMin) controls.noteHoldMsMin.value = String(pg.advanced.noteHoldMsMin);
        if (controls.noteHoldMsMax) controls.noteHoldMsMax.value = String(pg.advanced.noteHoldMsMax);
        if (controls.maxVoices) controls.maxVoices.value = String(pg.advanced.maxVoices);
        if (controls.spawnShape) controls.spawnShape.value = pg.advanced.spawnShape;
        if (pitchModeBtn) {
            const quant = pg.advanced.pitchQuantize !== false;
            pitchModeBtn.classList.toggle('toggle-on', quant);
            pitchModeBtn.classList.toggle('toggle-off', !quant);
            pitchModeBtn.textContent = quant ? 'PITCH SCALE' : 'PITCH FREE';
        }
        if (synthSafeBtn) {
            const safeOn = pg.advanced.internalSynthSafe !== false;
            synthSafeBtn.classList.toggle('toggle-on', safeOn);
            synthSafeBtn.classList.toggle('toggle-off', !safeOn);
            synthSafeBtn.textContent = safeOn ? 'SAFE ON' : 'SAFE OFF';
        }
        if (audioFollowBtn) {
            const on = pg.advanced.audioFollow !== false;
            audioFollowBtn.classList.toggle('toggle-on', on);
            audioFollowBtn.classList.toggle('toggle-off', !on);
            audioFollowBtn.textContent = on ? 'FOLLOW ON' : 'FOLLOW OFF';
        }
        if (streamBtn) {
            streamBtn.classList.toggle('toggle-on', !!pg.macro.stream);
            streamBtn.classList.toggle('toggle-off', !pg.macro.stream);
            streamBtn.textContent = pg.macro.stream ? 'STREAM ON' : 'STREAM OFF';
        }
        if (windQuickBtn) {
            const w = Number(pg.macro.wind || 0);
            const dir = w > 0.08 ? 'right' : (w < -0.08 ? 'left' : 'off');
            windQuickBtn.classList.toggle('toggle-on', Math.abs(w) > 0.08);
            windQuickBtn.classList.toggle('toggle-off', Math.abs(w) <= 0.08);
            windQuickBtn.title = `Wind ${w > 0.08 ? 'Right' : (w < -0.08 ? 'Left' : 'Off')}`;
            windQuickBtn.setAttribute('aria-label', windQuickBtn.title);
            windQuickBtn.dataset.windDir = dir;
        }
        if (controls.tubeMaterial) controls.tubeMaterial.value = pg.tubes.material;
        if (controls.tubeSurface) controls.tubeSurface.value = pg.tubes.surface;
        if (controls.tubeLength) controls.tubeLength.value = String(pg.tubes.length);
        if (controls.tubeDiameter) controls.tubeDiameter.value = String(pg.tubes.diameter);
        if (controls.tubeEndMode) controls.tubeEndMode.value = pg.tubes.endMode;
        if (controls.tubeMaxVoices) controls.tubeMaxVoices.value = String(pg.tubes.maxVoices);
        if (controls.tubeMaxCount) controls.tubeMaxCount.value = String(pg.tubes.maxCount);
        if (controls.tubeQuantizeStrength) controls.tubeQuantizeStrength.value = String(Math.round((pg.tubes.quantizeStrength ?? 0.7) * 100));
        if (controls.tubeSnapAngle) controls.tubeSnapAngle.value = String(pg.tubes.snapAngleDeg ?? 0);
        if (controls.tubeMood) controls.tubeMood.value = String(Math.round((pg.tubes.mood ?? 0.5) * 100));
        if (controls.tubeYExpress) controls.tubeYExpress.value = String(Math.round((pg.tubes.yExpressivity ?? 0.6) * 100));
        if (controls.tubeMute) {
            const on = !!pg.tubes.mute;
            controls.tubeMute.classList.toggle('toggle-on', on);
            controls.tubeMute.classList.toggle('toggle-off', !on);
            controls.tubeMute.textContent = on ? 'MUTE ON' : 'MUTE OFF';
        }
        if (controls.audioFollowDepth) controls.audioFollowDepth.value = String(pg.advanced.audioFollowDepth ?? 0.55);
        ...
        if (anchorNotesBtn) {
            const on = pg.advanced.anchorNotes === true;
            anchorNotesBtn.classList.toggle('toggle-on', on);
            anchorNotesBtn.classList.toggle('toggle-off', !on);
            anchorNotesBtn.textContent = on ? 'ANCHOR ON' : 'ANCHOR OFF';
        }
        if (rollingArpBtn) {
            const on = pg.advanced.rollingArp !== false;
            rollingArpBtn.classList.toggle('toggle-on', on);
            rollingArpBtn.classList.toggle('toggle-off', !on);
            rollingArpBtn.textContent = on ? 'ROLL ON' : 'ROLL OFF';
        }
        if (harmonicRainBtn) {
            const on = pg.advanced.harmonicRain !== false;
            harmonicRainBtn.classList.toggle('toggle-on', on);
            harmonicRainBtn.classList.toggle('toggle-off', !on);
            harmonicRainBtn.textContent = on ? 'RAIN ON' : 'RAIN OFF';
        }
        if (frictionDroneBtn) {
            const on = pg.advanced.frictionDrone !== false;
            frictionDroneBtn.classList.toggle('toggle-on', on);
            frictionDroneBtn.classList.toggle('toggle-off', !on);
            frictionDroneBtn.textContent = on ? 'DRONE ON' : 'DRONE OFF';
        }
        if (adaptiveMpeBtn) {
            const on = pg.advanced.adaptiveMpe !== false;
            adaptiveMpeBtn.classList.toggle('toggle-on', on);
            adaptiveMpeBtn.classList.toggle('toggle-off', !on);
            adaptiveMpeBtn.textContent = on ? 'MPE ADAPT' : 'MPE FIX';
        }
        if (noteFlashBtn) {
            const on = pg.advanced.noteFlash !== false;
            noteFlashBtn.classList.toggle('toggle-on', on);
            noteFlashBtn.classList.toggle('toggle-off', !on);
            noteFlashBtn.textContent = on ? 'FLASH ON' : 'FLASH OFF';
        }
        if (trailDegreeBtn) {
            const on = pg.advanced.trailByDegree === true;
            trailDegreeBtn.classList.toggle('toggle-on', on);
            trailDegreeBtn.classList.toggle('toggle-off', !on);
            trailDegreeBtn.textContent = on ? 'TRAIL ON' : 'TRAIL OFF';
        }
        if (rotationLineBtn) {
            const on = pg.advanced.showRotationLine === true;
            rotationLineBtn.classList.toggle('toggle-on', on);
            rotationLineBtn.classList.toggle('toggle-off', !on);
            rotationLineBtn.textContent = on ? 'LINE ON' : 'LINE OFF';
        }
        if (drawWallsBtn) {
            const on = String(pg.interaction?.mode || 'drawFling') === 'drawWalls';
            drawWallsBtn.classList.toggle('toggle-on', on);
            drawWallsBtn.classList.toggle('toggle-off', !on);
            drawWallsBtn.textContent = on ? 'WALL ON' : 'WALL OFF';
        }
        if (tubeQuantizeBtn) {
            tubeQuantizeBtn.classList.toggle('toggle-on', !!pg.tubes.quantize);
            tubeQuantizeBtn.classList.toggle('toggle-off', !pg.tubes.quantize);
            tubeQuantizeBtn.textContent = pg.tubes.quantize ? 'Q' : 'Q-';
        }
        if (tubeWindPlayBtn) {
            tubeWindPlayBtn.classList.toggle('toggle-on', !!pg.tubes.windPlay);
            tubeWindPlayBtn.classList.toggle('toggle-off', !pg.tubes.windPlay);
            tubeWindPlayBtn.textContent = pg.tubes.windPlay ? 'W' : 'W-';
        }
        if (tubeToolButtons.length) {
            const activeTool = String(pg.tubes.tool || 'select');
            tubeToolButtons.forEach((btn) => {
                const isOn = btn.dataset.tubeTool === activeTool;
                btn.classList.toggle('active', isOn);
                btn.setAttribute('aria-pressed', isOn ? 'true' : 'false');
            });
        }
        setTubeEditorVisible();
    };

    const clamp01Local = (v) => Math.max(0, Math.min(1, Number(v) || 0));
    const captureAudioFollowBaseline = () => {
        if (!state.audio) return;
        pgAudioFollowRuntime.baseline = {
            fxMix: state.audio.fx?.fxMix ?? 0.3,
            tubeDrive: state.audio.fx?.tubeDrive ?? 0.2,
            reverbWet: state.audio.fx?.reverbWet ?? 0.2,
            delayWet: state.audio.fx?.delayWet ?? 0.2,
            filterCutoff: state.audio.fx?.filterCutoff ?? 12000,
            crossModDepth: state.audio.crossMod?.depth ?? 0,
            macroTexture: state.audio.macro?.texture ?? 0.5
        };
        pgAudioFollowRuntime.armed = true;
    };

    const restoreAudioFollowBaseline = () => {
        if (!pgAudioFollowRuntime.armed || !pgAudioFollowRuntime.baseline || !state.audio) return;
        const b = pgAudioFollowRuntime.baseline;
        if (!state.audio.fx) state.audio.fx = { ...DEFAULT_FX };
        state.audio.fx.fxMix = clamp01Local(b.fxMix);
        state.audio.fx.tubeDrive = clamp01Local(b.tubeDrive);
        state.audio.fx.reverbWet = clamp01Local(b.reverbWet);
        state.audio.fx.delayWet = clamp01Local(b.delayWet);
        state.audio.fx.filterCutoff = Math.max(60, Math.min(20000, Number(b.filterCutoff) || 12000));
        if (typeof setCrossModDepth === 'function') setCrossModDepth(clamp01Local(b.crossModDepth));
        if (typeof setMacroTexture === 'function') setMacroTexture(clamp01Local(b.macroTexture));
        if (typeof applyFxToUI === 'function') applyFxToUI();
        if (typeof updateFxNodes === 'function') updateFxNodes();
        pgAudioFollowRuntime.armed = false;
        pgAudioFollowRuntime.baseline = null;
    };

    const applyPlaygroundAudioFollow = () => {
        const enabled = !!state.playground?.enabled;
        const follow = state.playground?.advanced?.audioFollow !== false;
        if (!enabled || !follow || !state.audio?.enabled) {
            restoreAudioFollowBaseline();
            return;
        }
        if (!pgAudioFollowRuntime.armed) captureAudioFollowBaseline();
        const b = pgAudioFollowRuntime.baseline;
        if (!b || !state.audio?.fx) return;

        const depth = clamp01Local(state.playground.advanced.audioFollowDepth ?? 0.55);
        const m = state.playground.macro || {};
        const energy = clamp01Local(m.energy ?? 0.6);
        const chaos = clamp01Local(m.chaos ?? 0.4);
        const gravity = clamp01Local(m.gravity ?? 0.8);
        const density = clamp01Local(m.density ?? 0.5);
        const wind = Math.max(-1, Math.min(1, Number(m.wind) || 0));
        const windAbs = Math.abs(wind);

        const nextFxMix = clamp01Local(b.fxMix + (((energy - 0.5) * 0.22) + density * 0.12) * depth);
        const nextTube = clamp01Local(b.tubeDrive + ((energy * 0.34) + (chaos * 0.15)) * depth);
        const nextRevWet = clamp01Local(b.reverbWet + ((windAbs * 0.3) + (density * 0.1)) * depth);
        const nextDelayWet = clamp01Local(b.delayWet + ((chaos * 0.22) + (energy * 0.08)) * depth);
        const cutoffMul = (0.72 + (1 - gravity) * 0.52 + energy * 0.16);
        const nextCutoff = Math.max(60, Math.min(20000, b.filterCutoff * (1 + (cutoffMul - 1) * depth)));
        const nextMod = clamp01Local(b.crossModDepth + ((chaos * 0.45) + windAbs * 0.18) * depth);
        const nextSpace = clamp01Local(b.macroTexture + ((windAbs * 0.52) + density * 0.2 - gravity * 0.12) * depth);

        state.audio.fx.fxMix = nextFxMix;
        state.audio.fx.tubeDrive = nextTube;
        state.audio.fx.reverbWet = nextRevWet;
        state.audio.fx.delayWet = nextDelayWet;
        state.audio.fx.filterCutoff = nextCutoff;
        if (typeof setCrossModDepth === 'function') setCrossModDepth(nextMod);
        if (typeof setMacroTexture === 'function') setMacroTexture(nextSpace);
        if (typeof applyFxToUI === 'function') applyFxToUI();
        if (typeof updateFxNodes === 'function') updateFxNodes();
    };

    const applyConfig = () => {
        state.playground = clampPlaygroundState(state.playground);
        Playground.setConfig(state.playground);
        ...
        Playground.setStream(!!state.playground.macro.stream && state.playground.mode !== 'tubes');
        applyPlaygroundAudioFollow();
        savePlaygroundStateToStorage();
        syncHudFromState();
    };

    const ensurePhysicsMode = () => {
        if (state.playground.mode !== 'physics') {
            state.playground.mode = 'physics';
            applyConfig();
        }
    };
    const setTubeTool = (tool) => {
        const allowed = ['select', 'place', 'play', 'cut', 'delete'];
        const next = allowed.includes(String(tool)) ? String(tool) : 'select';
        state.playground.tubes.tool = next;
        if (state.playground.mode !== 'tubes') state.playground.mode = 'tubes';
        applyConfig();
    };
    const PLAYGROUND_PRESETS = {
        rainPad: {
            patch: {
                mode: 'physics',
                macro: { energy: 0.35, chaos: 0.18, gravity: 0.56, density: 0.66, wind: 0.12, stream: true },
                advanced: { collisionThreshold: 0.9, mpeSmoothingMs: 32, noteHoldMsMin: 180, noteHoldMsMax: 1200, spawnShape: 'circle', pitchQuantize: true }
            },
            onApply: () => {
                ensurePhysicsMode();
                if (Playground?.spawnRain) {
                    Playground.spawnRain();
                    setTimeout(() => Playground.spawnRain(), 180);
                }
            }
        },
        glassBurst: {
            patch: {
                mode: 'physics',
                macro: { energy: 0.82, chaos: 0.58, gravity: 0.84, density: 0.44, wind: 0.04, stream: false },
                advanced: { collisionThreshold: 0.72, mpeSmoothingMs: 18, noteHoldMsMin: 90, noteHoldMsMax: 760, spawnShape: 'polygon', pitchQuantize: true }
            },
            onApply: () => {
                ensurePhysicsMode();
                Playground?.morph?.('GLASS');
                if (Playground?.spawnObject) {
                    Playground.spawnObject(72);
                    setTimeout(() => Playground.spawnObject(76), 120);
                    setTimeout(() => Playground.spawnObject(79), 240);
                }
            }
        },
        tubeDrone: {
            patch: {
                mode: 'tubes',
                macro: { energy: 0.42, chaos: 0.22, gravity: 0.22, density: 0.38, wind: 0.72, stream: false },
                advanced: { collisionThreshold: 1.05, mpeSmoothingMs: 28, noteHoldMsMin: 180, noteHoldMsMax: 1400, pitchQuantize: true },
                tubes: { tool: 'select', material: 'ALUMINUM', surface: 'smooth', length: 300, diameter: 30, endMode: 'open', quantize: true, windPlay: true, maxVoices: 8, maxCount: 28 }
            },
            onApply: () => {
                if (Playground?.getStats && Playground?.addTube) {
                    const stats = Playground.getStats();
                    if ((stats?.tubeCount || 0) === 0) {
                        const cv = document.getElementById('playgroundCanvas');
                        const w = cv?.width || (window.innerWidth * 0.5);
                        const h = cv?.height || (window.innerHeight * 0.5);
                        Playground.addTube({ x: w * 0.5, y: h * 0.5, material: 'ALUMINUM', surface: 'smooth', length: 300, diameter: 30, windPlay: true });
                    }
                }
            }
        },
        chaosPerc: {
            patch: {
                mode: 'physics',
                macro: { energy: 0.92, chaos: 0.9, gravity: 0.95, density: 0.8, wind: -0.36, stream: false },
                advanced: { collisionThreshold: 0.62, mpeSmoothingMs: 12, noteHoldMsMin: 70, noteHoldMsMax: 520, spawnShape: 'bar', pitchQuantize: false }
            },
            onApply: () => {
                ensurePhysicsMode();
                Playground?.morph?.('RUBBER');
                if (Playground?.spawnObject) {
                    for (let i = 0; i < 5; i += 1) {
                        setTimeout(() => Playground.spawnObject(), i * 90);
                    }
                }
            }
        }
    };
    const applyPlaygroundPreset = (presetKey) => {
        const preset = PLAYGROUND_PRESETS[presetKey];
        if (!preset) return;
        state.playground = clampPlaygroundState(deepMergePlain(state.playground, preset.patch));
        applyConfig();
        if (state.playground.enabled && typeof preset.onApply === 'function') {
            preset.onApply();
        }
    };
    const savePlaygroundScene = (nameRaw) => {
        const name = String(nameRaw || '').trim();
        if (!name) return false;
        const sceneData = Playground?.exportScene ? Playground.exportScene() : null;
        if (!sceneData) return false;
        playgroundScenes[name] = {
            name,
            savedAt: Date.now(),
            playground: JSON.parse(JSON.stringify(clampPlaygroundState(state.playground))),
            scene: sceneData
        };
        currentPlaygroundSceneName = name;
        savePlaygroundScenes();
        refreshPlaygroundSceneSelect(name);
        return true;
    };
    const loadPlaygroundScene = (nameRaw) => {
        const name = String(nameRaw || '').trim();
        if (!name || !playgroundScenes[name]) return false;
        const entry = playgroundScenes[name];
        if (!entry || typeof entry !== 'object') return false;
        suppressPlaygroundBootSpawn = true;
        try {
            if (!state.playground.enabled) setEnabled(true);
            state.playground = clampPlaygroundState(deepMergePlain(state.playground, entry.playground || {}));
            applyConfig();
            if (Playground?.importScene && entry.scene) {
                Playground.importScene(entry.scene);
            }
            currentPlaygroundSceneName = name;
            refreshPlaygroundSceneSelect(name);
            return true;
        } finally {
            suppressPlaygroundBootSpawn = false;
        }
    };
    const deletePlaygroundScene = (nameRaw) => {
        const name = String(nameRaw || '').trim();
        if (!name || !playgroundScenes[name]) return false;
        delete playgroundScenes[name];
        if (currentPlaygroundSceneName === name) currentPlaygroundSceneName = '';
        savePlaygroundScenes();
        refreshPlaygroundSceneSelect('');
        return true;
    };
    const normalizeImportedPlaygroundScenes = (raw) => {
        if (!raw || typeof raw !== 'object') return {};
        let source = raw;
        if (raw.format === 'genca-playground-scenes' && raw.scenes && typeof raw.scenes === 'object') {
            source = raw.scenes;
        }
        const out = {};
        Object.keys(source).forEach((nameRaw) => {
            const name = String(nameRaw || '').trim();
            if (!name) return;
            const entry = source[nameRaw];
            if (!entry || typeof entry !== 'object') return;
            const pg = clampPlaygroundState(entry.playground || {});
    ...
    const exportPlaygroundScenesToFile = () => {
        const payload = {
            format: 'genca-playground-scenes',
            version: 1,
            exportedAt: Date.now(),
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `genca-playground-scenes-${stamp}.json`;
        document.body.appendChild(a);
        a.click();
        a.remove();
        URL.revokeObjectURL(url);
    };
    const importPlaygroundScenesFromFile = async (file) => {
        if (!file) return false;
        try {
            const text = await file.text();
            const parsed = safeParseJson(text, null);
            const normalized = normalizeImportedPlaygroundScenes(parsed);
            const names = Object.keys(normalized);
            if (!names.length) return false;
            playgroundScenes = normalized;
            currentPlaygroundSceneName = '';
            savePlaygroundScenes();
            refreshPlaygroundSceneSelect('');
            return true;
        } catch (_) {
    ...
    const setEnabled = (enabled) => {
        state.playground.enabled = !!enabled;
        window.state = state;
        if (state.playground.enabled) {
            ...
            Playground.setStream(!!state.playground.macro.stream && state.playground.mode !== 'tubes');
            setTubeEditorVisible();
            const onboardingSeen = localStorage.getItem(PG_ONBOARD_SEEN_KEY) === '1';
            if (!onboardingSeen) {
                setTimeout(() => {
                    if (state.playground?.enabled) showPlaygroundOnboarding();
                }, 260);
            }
        } else {
            ...
            if (pgControls) pgControls.style.display = 'none';
    ...
    if (matBtns.length > 0) {
        matBtns.forEach((btn) => {
            btn.addEventListener('click', (e) => {
                ...
                const target = e.target.closest('.mat-btn');
                if (!target) return;
                target.classList.add('active');
                Playground.morph(target.dataset.mat);
            });
        });
    }
    ...
    if (btnSpawn) btnSpawn.addEventListener('click', () => {
        if (!Playground.isRunning) return;
        ensurePhysicsMode();
        Playground.spawnObject();
    });
    if (btnClear) btnClear.addEventListener('click', () => {
        if (!Playground.isRunning) return;
        if (state.playground.mode === 'tubes' && Playground?.clearTubes) Playground.clearTubes();
        else Playground.clearScene({ keepEnvironment: true, keepWalls: true });
    });
    if (windQuickBtn) {
        windQuickBtn.addEventListener('click', () => {
            const w = Number(state.playground.macro.wind || 0);
            if (w >= 0.08) state.playground.macro.wind = -0.7;
            else if (w <= -0.08) state.playground.macro.wind = 0;
            else state.playground.macro.wind = 0.7;
            applyConfig();
        });
    }
    if (tubeEditorToggleBtn) {
        tubeEditorToggleBtn.addEventListener('click', () => {
            if (state.playground.mode !== 'tubes') {
                state.playground.mode = 'tubes';
                tubeEditorOpen = true;
                applyConfig();
            ...
            setTubeEditorVisible(!tubeEditorOpen);
        });
    }
    if (tubeEditorCloseBtn) {
        tubeEditorCloseBtn.addEventListener('click', () => {
            setTubeEditorVisible(false);
        });
    }
    if (tubeTemplateBtns.length) {
        tubeTemplateBtns.forEach((btn) => {
            btn.addEventListener('click', () => {
                const tpl = parseTubeTemplateFromEl(btn);
                if (!tpl) return;
                const isSame = armedTubeTemplateBtn === btn;
                if (isSame) setArmedTubeTemplate(null, null);
                else setArmedTubeTemplate(tpl, btn);
            });
            btn.addEventListener('dragstart', (ev) => {
                const tpl = parseTubeTemplateFromEl(btn);
                if (!tpl || !ev.dataTransfer) return;
                setArmedTubeTemplate(tpl, btn);
                ev.dataTransfer.effectAllowed = 'copy';
                ev.dataTransfer.setData('application/x-genca-tube-template', JSON.stringify(tpl));
            });
        });
    }
    if (tubeToolButtons.length) {
        tubeToolButtons.forEach((btn) => {
            btn.addEventListener('click', () => {
                setTubeTool(btn.dataset.tubeTool || 'select');
            });
        });
    }
    if (pgPresetButtons.length) {
        pgPresetButtons.forEach((btn) => {
            btn.addEventListener('click', () => {
                applyPlaygroundPreset(btn.dataset.pgPreset || '');
            });
        });
    }
    if (pgSceneSelect) {
        pgSceneSelect.addEventListener('change', () => {
            if (pgSceneName && pgSceneSelect.value) pgSceneName.value = pgSceneSelect.value;
            currentPlaygroundSceneName = pgSceneSelect.value || '';
            updatePlaygroundSceneStatus();
        });
    }
    if (pgSceneSaveBtn) {
        pgSceneSaveBtn.addEventListener('click', () => {
            const name = (pgSceneName?.value || pgSceneSelect?.value || '').trim();
            if (!name) return;
            const ok = savePlaygroundScene(name);
            if (ok && pgSceneName) pgSceneName.value = name;
        });
    }
    if (pgSceneLoadBtn) {
        pgSceneLoadBtn.addEventListener('click', () => {
            const name = (pgSceneSelect?.value || pgSceneName?.value || '').trim();
            if (!name) return;
            loadPlaygroundScene(name);
        });
    }
    if (pgSceneDeleteBtn) {
        pgSceneDeleteBtn.addEventListener('click', () => {
            const name = (pgSceneSelect?.value || pgSceneName?.value || '').trim();
            if (!name) return;
            const ok = deletePlaygroundScene(name);
            if (ok && pgSceneName) pgSceneName.value = '';
        });
    }
    if (pgSceneExportBtn) {
        pgSceneExportBtn.addEventListener('click', () => {
            exportPlaygroundScenesToFile();
        });
    }
    if (pgSceneImportBtn && pgSceneFile) {
        pgSceneImportBtn.addEventListener('click', () => {
            pgSceneFile.value = '';
            pgSceneFile.click();
        });
        pgSceneFile.addEventListener('change', async (ev) => {
            const file = ev.target?.files?.[0];
            const ok = await importPlaygroundScenesFromFile(file);
            if (ok && pgSceneName) pgSceneName.value = '';
        });
    }
    if (pgOnboardPrev) {
        pgOnboardPrev.addEventListener('click', () => {
            pgOnboardStep = Math.max(0, pgOnboardStep - 1);
            renderPgOnboardStep();
        });
    }
    if (pgOnboardNext) {
        pgOnboardNext.addEventListener('click', () => {
            pgOnboardStep = Math.min(PG_ONBOARD_STEPS.length - 1, pgOnboardStep + 1);
            renderPgOnboardStep();
        });
    }
    if (pgOnboardDone) pgOnboardDone.addEventListener('click', () => closePlaygroundOnboarding(true));
    if (pgOnboardClose) pgOnboardClose.addEventListener('click', () => closePlaygroundOnboarding(true));
    if (playgroundCanvas) {
        playgroundCanvas.addEventListener('dragover', (ev) => {
            if (!state.playground?.enabled) return;
            ev.preventDefault();
            if (ev.dataTransfer) ev.dataTransfer.dropEffect = 'copy';
        });
        playgroundCanvas.addEventListener('drop', (ev) => {
            if (!state.playground?.enabled) return;
            ev.preventDefault();
            let tpl = armedTubeTemplate;
            const raw = ev.dataTransfer ? ev.dataTransfer.getData('application/x-genca-tube-template') : '';
            if (raw) {
                try {
                    tpl = JSON.parse(raw);
                } catch (_) {
                    tpl = armedTubeTemplate;
                }
            }
            const placed = spawnTubeAtClientPoint(ev.clientX, ev.clientY, tpl);
            if (placed) setArmedTubeTemplate(null, null);
        });
        // Touch fallback: tap template then tap canvas to place one tube.
        playgroundCanvas.addEventListener('pointerdown', (ev) => {
            if (!armedTubeTemplate || !state.playground?.enabled) return;
            const placed = spawnTubeAtClientPoint(ev.clientX, ev.clientY, armedTubeTemplate);
            if (!placed) return;
            setArmedTubeTemplate(null, null);
            ev.preventDefault();
            ev.stopPropagation();
    ...
    if (pitchModeBtn) {
        pitchModeBtn.addEventListener('click', () => {
            state.playground.advanced.pitchQuantize = !state.playground.advanced.pitchQuantize;
            applyConfig();
        });
    }
    if (synthSafeBtn) {
        synthSafeBtn.addEventListener('click', () => {
            state.playground.advanced.internalSynthSafe = !state.playground.advanced.internalSynthSafe;
            applyConfig();
        });
    }
    if (audioFollowBtn) {
        audioFollowBtn.addEventListener('click', () => {
            state.playground.advanced.audioFollow = !state.playground.advanced.audioFollow;
            applyConfig();
        });
    }
    if (anchorNotesBtn) {
        anchorNotesBtn.addEventListener('click', () => {
            state.playground.advanced.anchorNotes = !state.playground.advanced.anchorNotes;
            applyConfig();
        });
    }
    if (rollingArpBtn) {
        rollingArpBtn.addEventListener('click', () => {
            state.playground.advanced.rollingArp = !state.playground.advanced.rollingArp;
            applyConfig();
        });
    }
    if (harmonicRainBtn) {
        harmonicRainBtn.addEventListener('click', () => {
            state.playground.advanced.harmonicRain = !state.playground.advanced.harmonicRain;
            applyConfig();
        });
    }
    if (frictionDroneBtn) {
        frictionDroneBtn.addEventListener('click', () => {
            state.playground.advanced.frictionDrone = !state.playground.advanced.frictionDrone;
            applyConfig();
        });
    }
    if (adaptiveMpeBtn) {
        adaptiveMpeBtn.addEventListener('click', () => {
            state.playground.advanced.adaptiveMpe = !state.playground.advanced.adaptiveMpe;
            applyConfig();
        });
    }
    if (noteFlashBtn) {
        noteFlashBtn.addEventListener('click', () => {
            state.playground.advanced.noteFlash = !state.playground.advanced.noteFlash;
            applyConfig();
        });
    }
    if (trailDegreeBtn) {
        trailDegreeBtn.addEventListener('click', () => {
            state.playground.advanced.trailByDegree = !state.playground.advanced.trailByDegree;
            applyConfig();
        });
    }
    if (rotationLineBtn) {
        rotationLineBtn.addEventListener('click', () => {
            state.playground.advanced.showRotationLine = !state.playground.advanced.showRotationLine;
            applyConfig();
        });
    }
    if (drawWallsBtn) {
        drawWallsBtn.addEventListener('click', () => {
            ensurePhysicsMode();
            const next = state.playground.interaction?.mode === 'drawWalls' ? 'drawFling' : 'drawWalls';
            state.playground.interaction.mode = next;
            applyConfig();
        });
    }
    if (clearWallsBtn) {
        clearWallsBtn.addEventListener('click', () => {
            if (Playground?.clearWalls) Playground.clearWalls();
        });
    }
    if (pgLoopClearBtn) {
        pgLoopClearBtn.addEventListener('click', () => {
            stopAllPgLoops();
        });
    }
    if (pgLoopBtn) {
        pgLoopBtn.addEventListener('click', () => {
            if (!state.playground?.enabled) return;
    ...
    if (btnAdvancedToggle && advancedPanel) {
        btnAdvancedToggle.addEventListener('click', () => {
            ...
            btnAdvancedToggle.classList.toggle('toggle-on', willOpen);
            btnAdvancedToggle.classList.toggle('toggle-off', !willOpen);
            btnAdvancedToggle.title = willOpen ? 'Hide settings' : 'Open settings';
            if (!willOpen && state.playground?.interaction?.mode === 'drawWalls') {
    ...
    if (pgTabButtons.length) {
        pgTabButtons.forEach((btn) => {
            btn.addEventListener('click', () => {
                setPgTab(btn.dataset.pgTab || 'playability');
            });
        });
        setPgTab('playability');
    }

    const bindNumber = (el, path, parser = parseFloat) => {
        if (!el) return;
        el.addEventListener('input', () => {
            const parts = path.split('.');
            const key = parts.pop();
            let root = state.playground;
            parts.forEach((p) => { root = root[p]; });
            root[key] = parser(el.value);
            applyConfig();
        });
    };

    bindNumber(controls.energy, 'macro.energy');
    bindNumber(controls.chaos, 'macro.chaos');
    bindNumber(controls.gravity, 'macro.gravity');
    bindNumber(controls.density, 'macro.density');
    bindNumber(controls.wind, 'macro.wind');
    bindNumber(controls.collisionThreshold, 'advanced.collisionThreshold');
    bindNumber(controls.mpeSmoothingMs, 'advanced.mpeSmoothingMs', (v) => parseInt(v, 10));
    bindNumber(controls.noteHoldMsMin, 'advanced.noteHoldMsMin', (v) => parseInt(v, 10));
    bindNumber(controls.noteHoldMsMax, 'advanced.noteHoldMsMax', (v) => parseInt(v, 10));
    bindNumber(controls.maxVoices, 'advanced.maxVoices', (v) => parseInt(v, 10));
    bindNumber(controls.audioFollowDepth, 'advanced.audioFollowDepth');
    bindNumber(controls.tubeQuantizeStrength, 'tubes.quantizeStrength', (v) => Math.max(0, Math.min(1, (parseInt(v, 10) || 0) / 100)));
    bindNumber(controls.tubeSnapAngle, 'tubes.snapAngleDeg', (v) => parseInt(v, 10));
    bindNumber(controls.tubeMood, 'tubes.mood', (v) => Math.max(0, Math.min(1, (parseInt(v, 10) || 0) / 100)));
    bindNumber(controls.tubeYExpress, 'tubes.yExpressivity', (v) => Math.max(0, Math.min(1, (parseInt(v, 10) || 0) / 100)));
    bindNumber(controls.timingGrid, 'advanced.timingGridMs', (v) => parseInt(v, 10));
    bindNumber(controls.swing, 'advanced.swing');
    bindNumber(controls.maxNotes, 'advanced.maxNotesPerSec', (v) => parseInt(v, 10));
    bindNumber(controls.pitchLanes, 'advanced.pitchLanes', (v) => parseInt(v, 10));
    bindNumber(controls.loopQuantizeStrength, 'advanced.loopQuantizeStrength', (v) => Math.max(0, Math.min(1, (parseInt(v, 10) || 0) / 100)));
    bindNumber(controls.impactTimbre, 'advanced.impactToTimbre');
    bindNumber(controls.droneLevel, 'advanced.droneLevel');

    if (controls.spawnShape) {
        controls.spawnShape.addEventListener('change', () => {
            state.playground.advanced.spawnShape = controls.spawnShape.value;
            applyConfig();
        });
    }
    ...
    if (streamBtn) {
        streamBtn.addEventListener('click', () => {
            state.playground.macro.stream = !state.playground.macro.stream;
            applyConfig();
        });
    }
    if (tubeQuantizeBtn) {
        tubeQuantizeBtn.addEventListener('click', () => {
            state.playground.tubes.quantize = !state.playground.tubes.quantize;
            if (Playground?.updateSelectedTube) Playground.updateSelectedTube({ quantize: state.playground.tubes.quantize }, { preview: false });
            applyConfig();
        });
    }
    if (tubeWindPlayBtn) {
        tubeWindPlayBtn.addEventListener('click', () => {
            state.playground.tubes.windPlay = !state.playground.tubes.windPlay;
            if (Playground?.updateSelectedTube) Playground.updateSelectedTube({ windPlay: state.playground.tubes.windPlay }, { preview: false });
            applyConfig();
        });
    }
    if (tubeMuteBtn) {
        tubeMuteBtn.addEventListener('click', () => {
            state.playground.tubes.mute = !state.playground.tubes.mute;
            applyConfig();
        });
    }
    if (tubeClearBtn) {
        tubeClearBtn.addEventListener('click', () => {
            if (Playground?.clearTubes) Playground.clearTubes();
        });
    }

    const bindTubeField = (el, key, parser = (v) => v) => {
        if (!el) return;
        const evt = el.tagName === 'SELECT' ? 'change' : 'input';
        el.addEventListener(evt, () => {
            const parsed = parser(el.value);
            state.playground.tubes[key] = parsed;
            const canApplyToSelected = ['material', 'surface', 'length', 'diameter', 'endMode', 'quantize', 'windPlay'].includes(key);
            if (canApplyToSelected && Playground?.updateSelectedTube) {
                const patch = {};
                patch[key] = parsed;
                // Preview solo su cambi timbrici/intonativi principali.
                const preview = ['material', 'surface', 'length', 'diameter', 'endMode'].includes(key);
                Playground.updateSelectedTube(patch, { preview });
            }
            applyConfig();
        });
    };
    bindTubeField(controls.tubeMaterial, 'material', (v) => String(v || 'PVC'));
    bindTubeField(controls.tubeSurface, 'surface', (v) => String(v || 'corrugated'));
    bindTubeField(controls.tubeLength, 'length', (v) => parseInt(v, 10));
    bindTubeField(controls.tubeDiameter, 'diameter', (v) => parseInt(v, 10));
    bindTubeField(controls.tubeEndMode, 'endMode', (v) => (v === 'closed' ? 'closed' : 'open'));
    bindTubeField(controls.tubeMaxVoices, 'maxVoices', (v) => parseInt(v, 10));
    bindTubeField(controls.tubeMaxCount, 'maxCount', (v) => parseInt(v, 10));

    refreshPlaygroundSceneSelect(pgSceneSelect?.value || '');
    renderPgOnboardStep();
    closePlaygroundOnboarding(false);
    syncHudFromState();
    applyConfig();
    ...
    setInterval(() => {
        ...
        const s = Playground.getStats ? Playground.getStats() : null;
        if (!s) return;
        voicesEl.textContent = `Voices: ${s.activeVoices}`;
        cpuEl.textContent = `CPU: ${Math.max(1, Math.round(s.avgFrameMs))}ms / ${s.fpsHint}fps`;
        const near = state.playground?.advanced?.maxVoices ? s.activeVoices >= (state.playground.advanced.maxVoices - 2) : false;
        voicesEl.classList.toggle('near-limit', !!near);
        applyPlaygroundAudioFollow();
    }, 220);
});
